                ...
                {['Provinsi','Kabupaten/Kota','Kecamatan'].map(lbl => (
                    <div key={lbl}>
                        ...
                        <div className="relative">
                            <select className="w-full h-11 pl-4 pr-8 border border-slate-200 rounded-lg text-xs text-slate-400 appearance-none focus:border-[#0080C5] focus:outline-none transition-all bg-white">
                                <option>Pilih...</option>
                            </select>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2">
                        <input type="time" value={data.waktuMulai || ''} onChange={e => onChange('waktuMulai', e.target.value)} className="flex-1 h-11 px-4 border border-slate-200 rounded-lg text-xs focus:border-[#0080C5] focus:outline-none transition-all bg-white" />
                        <span className="text-slate-400 text-xs font-medium">s/d</span>
                        <input type="time" value={data.waktuSelesai || ''} onChange={e => onChange('waktuSelesai', e.target.value)} className="flex-1 h-11 px-4 border border-slate-200 rounded-lg text-xs focus:border-[#0080C5] focus:outline-none transition-all bg-white" />
                    </div>
                </div>
            </div>
            <div>
    ...
    return (
        <AdminLayout title="Kelola Kegiatan">
            <div className="font-['Poppins'] bg-[#F0F2F8] min-h-screen text-left">
                ...
                {/* Top Banner Mobile */}
                <div className="lg:hidden bg-[#0080C5] rounded-t-[16px] px-4 py-4 flex items-center gap-3">
                    <div className="w-9 h-9 bg-white/20 rounded-xl flex items-center justify-center shrink-0">
                        <UserCircle2 size={18} className="text-white" />
                    </div>
                    <div className="flex flex-col">
                        <h2 className="text-white text-[13px] font-bold">{isEdit ? 'Edit Kegiatan' : 'Tambah Kegiatan'}</h2>
                        <p className="text-white/70 text-[10px] leading-snug">
                            {step === 1 ? 'Lengkapi informasi kegiatan dampingan' : step === 2 ? 'Absensi peserta kegiatan' : 'Dokumentasi & Laporan Kegiatan'}
                        </p>
                    </div>
                </div>
