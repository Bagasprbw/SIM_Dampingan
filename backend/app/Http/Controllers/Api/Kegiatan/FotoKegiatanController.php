...
class FotoKegiatanController extends Controller
{
    public function index($kegiatanId)
    {
        ...
        $fotos = FotoKegiatan::where('kegiatan_id', $kegiatanId)
            ->orderBy('created_at', 'desc')
            ->get();

        return response()->json([
            'message' => 'Data foto kegiatan berhasil diambil',
            'data' => $fotos,
        ]);
    }
    ...
    public function store(Request $request, $kegiatanId)
    {
        ...
        if (! $request->hasFile('file') && ! $request->hasFile('files')) {
            return response()->json([
                'message' => 'Minimal unggah 1 file foto kegiatan',
            ], 422);
        }
        ...
        foreach ($uploadedFiles as $file) {
            $storedPath = $file->store('kegiatan/foto_kegiatan', 'public');
            $created[] = FotoKegiatan::create([
    ...
    public function destroy($kegiatanId, $idFoto)
    {
        ...
        if (! $foto) {
            return response()->json(['message' => 'Foto kegiatan tidak ditemukan'], 404);
        }
